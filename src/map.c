...
void save_initial_map(sokoban_t *skb)
{
    int pos[2] = {0};

    skb->initial = malloc(sizeof(char *) * skb->lines);
    for (int i = 0; i < skb->lines; i++)
        skb->initial[i] = malloc(sizeof(char) * (skb->length + 1));
    for (int i = 0; skb->buffer[i] != '\0'; i++, pos[0]++) {
        if (skb->buffer[i] == '\n') {
            skb->initial[pos[1]][pos[0]] = '\0';
            pos[1] ++;
            pos[0] = -1;
            continue;
        }
        ...
        skb->initial[pos[1]][pos[0]] = skb->buffer[i];
    }
    skb->initial[pos[1]][pos[0]] = '\0';
}
