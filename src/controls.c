...
void init_controls(sokoban_t *skb)
{
    ...
    skb->controls[2] = setup_control(KEY_UP, up_movement);
    skb->controls[3] = setup_control(KEY_DOWN, down_movement);
    skb->controls[4] = setup_control(KEY_LEFT, left_movement);
    skb->controls[5] = setup_control(KEY_RIGHT, right_movement);
}
